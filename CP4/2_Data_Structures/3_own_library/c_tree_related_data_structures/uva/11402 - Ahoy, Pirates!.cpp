    ...
    private:
        ...
        void flipoperation(int p, int L, int R) {
            if (flip[p] != -1) {
                st[p] = (R-L-st[p]+1);
                if (L != R) {
                    if (lazy[l(p)] != -1) lazy[l(p)] = 1-lazy[l(p)];
                    else flip[l(p)] = (flip[l(p)] == 1)? -1 : 1;

                    if (lazy[r(p)] != -1) lazy[r(p)] = 1-lazy[r(p)];
                    else flip[r(p)] = (flip[r(p)] == 1)? -1 : 1;
                } else {
                    A[L] = st[p];            
                }
                flip[p] = -1; 
...
void solve(int z) {
    string P; P.reserve(16777216);
    int N; cin >> N;
    for (int i=0; i<N; i++) {
        int C; string Part; cin >> C >> Part;
        while (C--) P+=Part;
    }
    P.shrink_to_fit();
    int S = sbpow(SIZE(P));
    vi A; A.reserve(S);
    // cout << P << "\n";
    for (char x: P) A.PB((x-'0'));
    for (int i=SIZE(A); i<S; i++) A.PB(0);
    // for (int x: A) cout << x << " ";
    // cout << "\n";
    SegmentTree St(A);
    cout << "Case " << z << ":\n";
    ...
    for (int i=0; i<Q; i++) {
        ...
        int x, y; cin >> x >> y;
        if (action == 'F') St.update(x, y, 1);
        else if (action == 'E') St.update(x, y, 0);
        else if (action == 'I') St.flips(x, y);
        else cout << "Q" << q++ << ": " << St.RSQ(x, y) << "\n";
    }
}
